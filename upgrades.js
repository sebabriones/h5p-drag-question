    ...
    1: {
      1: {
        contentUpgrade: function (parameters, finished, extras) {
          var migrateCfrd10To11 = function (params, done) {
            var settings = params.question && params.question.settings;

            if (params.instructions && params.instructions.appearance) {
              delete params.instructions.appearance;
            }

            if (settings && settings.instructions && !params.instructions) {
              params.instructions = settings.instructions;
            }
            ...
            if (params.behaviour.shuffleInitialPositions === undefined) {
              params.behaviour.shuffleInitialPositions = false;
            }

            if (params.behaviour.dragHandleVisibility === undefined) {
              params.behaviour.dragHandleVisibility = true;
            }
            ...
            done(null, params, extras);
          };

          if (parameters.behaviour && parameters.behaviour.dropZoneHighlighting !== undefined) {
            migrateCfrd10To11(parameters, finished);
            return;
          }
